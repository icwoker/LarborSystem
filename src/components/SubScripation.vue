    ...
    setup(){
        ...
        // },
            ...
            function getSet(){
                ...
                // const setUrl = 'https://wx-app.wangyuan.info/ldk/api/user';
                ...
                axios({
                    ...
                    response => {
                        ...
                        alert('提交成功');
                        // alert(response.data.code);
                        localStorage.setItem('isSubscribe', official.value);
                        localStorage.setItem('isMailSubscribe',email.value);
                    }
                    ,
                    error =>{
                        console.log(error.massage);
            ...
            // const tokenUrl = ' https://wx-app.wangyuan.info/ldk/api/callback' + '?code=' + code;
            axios({
                url:  tokenUrl,
                method:'get'
            }).then(
        ...
        setTimeout(() => {
        window.location.href = ' https://wx-app.wangyuan.info/ldk/api/wxLogin';
        }, 1000);
    }
        alert('这里没有被重定向');
        if(window.location.href.indexOf('code') >= 0)
        {
            ...
            const codeU = n.split('=');
            const code = codeU[codeU.length - 1];
            const tokenUrl = 'http://localhost:8080/callback' + '?code=' + code;
            // const tokenUrl = ' https://wx-app.wangyuan.info/ldk/api/callback' + '?code=' + code;
            axios({
                url:tokenUrl,
                method: 'get'
                ...
                response=>{
                    alert('成功');
                    // alert(response.data.data.isSubscribe);
                    console.log(response.data.data.data.isSubscribe);
                    console.log(response.data.data.data.isMailSubscribe);
                    // setTimeout(() => {
                    //     alert(response.data.data.data.isSubscribe);
                    // }, 3000);
                    let isofficial = Boolean(response.data.data.data.isSubscribe >= 1);
                    let ismail = Boolean(response.data.data.data.isMailSubscribe >= 1);
                    // console.log(isofficial);
                    // console.log(ismail);
                    
                    official.value = isofficial?true:false;
                    email.value = ismail?true:false;
                },
                error=>{
                    alert('失败');
                    alert(error.massage);
